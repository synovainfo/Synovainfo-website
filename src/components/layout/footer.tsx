...
function ColumnHeading({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="mb-5 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-400">
      {children}
    </h3>
  )
}
...
export function Footer() {
  ...
  return (
    <footer
      className="relative overflow-hidden border-t border-zinc-800/60 bg-[#080E18]"
      role="contentinfo"
    >
      {/* Subtle top glow */}
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse at center, var(--color-accent-blue) 0%, transparent 70%)',
        }}
        aria-hidden
      />
                ...
                {COMPANY_LINKS.map((link) => (
                  <li key={link.label}>
                    ...
                    >
                      <ChevronRight
                        size={12}
                        className="shrink-0 text-zinc-600 opacity-0 transition-all duration-200 group-hover:opacity-100 group-hover:text-accent-blue"
                        aria-hidden
                      />
              ...
              {CONTACT_INFO.map((item) => {
                ...
                const wrapperProps = item.href
                  ? {
                      href: item.href,
                      className:
                        'group inline-flex items-center gap-3 text-sm text-zinc-400 transition-colors duration-200 hover:text-white',
                    }
                  : {
                      className:
                ...
                return (
                  <li key={item.label}>
                    <Wrapper {...wrapperProps}>
                      <Icon
                        size={16}
                        className="shrink-0 text-zinc-500 group-hover:text-accent-blue"
                        aria-hidden
                      />
              ...
              <form onSubmit={handleNewsletterSubmit} className="relative">
                <div className="relative">
                  <input
                    ...
                    className={cn(
                      'w-full rounded-lg border bg-zinc-900/50 px-3.5 py-2.5 pr-10 text-sm text-white placeholder-zinc-500 outline-none transition-all duration-200',
                      newsletterStatus === 'error'
                        ? 'border-red-500/60 focus:border-red-500'
                        : 'border-zinc-700/60 focus:border-accent-blue/60',
                    )}
                  />
                  <button
                    type="submit"
                    aria-label="Subscribe"
                    className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded-md p-1.5 text-zinc-500 transition-all duration-200 hover:text-accent-blue hover:bg-zinc-800/80"
                  >
        ...
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
              ...
              <span className="hidden text-zinc-700/50 sm:inline" aria-hidden>|</span>
